...
class BackupManager(
    ...
    private fun extraerYValidar(zip: File, destino: File) {
        ...
        ZipInputStream(FileInputStream(zip)).use { entrada ->
            ...
            while (item != null) {
                ...
                require(nombre == "manifest.properties" || nombre.startsWith("database/") || nombre.startsWith("cotizaciones/") || nombre.startsWith("evidencias/")) { "El respaldo contiene rutas no permitidas." }
        ...
        require((manifest?.getProperty("roomVersion")?.toIntOrNull() ?: 0) <= 7) { "El respaldo fue creado con una versión más reciente de la aplicación." }
    }
    ...
    private fun restaurarRecuperacion(origen: File) {
        ...
        reemplazarCarpeta(File(origen, "cotizaciones"), File(raiz, "cotizaciones")); reemplazarCarpeta(File(origen, "evidencias"), File(raiz, "evidencias"))
    }
