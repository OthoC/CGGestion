    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            estado.error?.let { mensaje ->
                item(key = "error-reportes") {
                    Card(colors = CardDefaults.cardColors(containerColor = FondoTarjeta)) {
                        Text(mensaje, color = Color.White, modifier = Modifier.padding(12.dp))
                    }
                }
            }
            item(key = "periodos") {
                FlowRow(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                    PeriodoReporte.entries.forEach { periodo ->
                        OutlinedButton(onClick = { viewModel.seleccionarPeriodo(periodo) }) {
                            Text(periodo.name, color = if (periodo == estado.periodo) RojoCG else Color.White)
                        }
                    }
                }
            }
            ...
            items(cotizaciones.take(5), key = { "cotizacion-${it.cotizacion.id}" }) { cotizacion ->
                ItemReporte(
                    ...
                    "${cotizacion.clienteNombre} · ${cotizacion.cotizacion.estado}",
                    moneda(cotizacion.cotizacion.totalFinalCentavos)
                )
            }
            item(key = "titulo-hojas") {
                Text("HOJAS RECIENTES", color = RojoCG, modifier = Modifier.padding(top = 8.dp))
            }
            items(hojas.take(5), key = { "hoja-${it.hoja.id}" }) { hoja ->
                ItemReporte(
                    hoja.hoja.numeroHoja,
                    "${hoja.clienteNombre} · ${hoja.hoja.estado}",
                    "${hoja.evidenciasCantidad} foto(s)"
                )
            }
            item(key = "titulo-inventario") {
                Text("ALERTAS DE INVENTARIO", color = RojoCG, modifier = Modifier.padding(top = 8.dp))
            }
            items(estado.reporte.stockBajo, key = { "producto-${it.id}" }) { producto ->
                ItemReporte(
                    producto.nombre,
...
@Composable
private fun TarjetaResumen(titulo: String, valor: String, modifier: Modifier) {
    Card(modifier, colors = CardDefaults.cardColors(containerColor = FondoTarjeta)) {
...
@Composable
private fun ItemReporte(titulo: String, detalle: String, valor: String) {
    Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = FondoTarjeta)) {
        Row(Modifier.fillMaxWidth().padding(12.dp), horizontalArrangement = Arrangement.SpaceBetween) {
            Column(Modifier.weight(1f)) {
                Text(titulo, color = Color.White)
                Text(detalle, color = TextoSecundario)
            }
            Text(valor, color = Color.White)
        }
    }
